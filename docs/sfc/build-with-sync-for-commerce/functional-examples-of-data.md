...
We include some examples of how the data is represented **in the accounting platforms’ UI** in our [Xero mapping specification](/sfc/mapping-specifications/xero-mapping-specification) and [QuickBooks Online mapping specification](/sfc/mapping-specifications/qbo-mapping-specification).
...
To check whether the data has been mapped to the accounting platform successfully, follow the steps in [Push commerce data to Codat](/sfc/build-with-sync-for-commerce/sync-data-pushing).
:::
...
1. Push the [Orders](/commerce-api#/schemas/orders) that represent any Sales made during the day. See [Example 1](/sfc/build-with-sync-for-commerce/functional-examples-of-data#example-1-an-order-representing-a-sale).
2. Push the [Payments](/commerce-api#/schemas/payments) associated with the Orders from Step 1. See [Example 2](/sfc/build-with-sync-for-commerce/functional-examples-of-data#example-2-a-payment-associated-with-the-order-from-example-1).
3. Push the [Transactions](/commerce-api#/schemas/transactions) that will enable reconciliation of the Orders from Step 1 with the Payments from Step 2. See [Example 3](/sfc/build-with-sync-for-commerce/functional-examples-of-data#example-3-a-transaction-that-reconciles-the-order-from-example-1-and-the-payment-from-example-2).
   Codat reconciles Payments and Orders using the `id` property, as shown in [Example 3](/sfc/build-with-sync-for-commerce/functional-examples-of-data#example-3-a-transaction-that-reconciles-the-order-from-example-1-and-the-payment-from-example-2).
4. Push the [Transactions](/commerce-api#/schemas/transactions) that represent the commerce service [Payment fees](/sfc/mapping-specifications/xero-mapping-specification#payment-fees) associated with the submitted Payments and Orders. This allows for reconciliation with the corresponding Payments and Orders. See [Example 4](/sfc/build-with-sync-for-commerce/functional-examples-of-data#example-4-a-transaction-that-reconciles-the-order-from-example-1-and-the-payment-of-commerce-provider-fees-associated-with-this-order).
   Note that this only applies if you receive processing fees from your SMBs.
   Codat reconciles the Payment fee Transaction with the associated Payments and Orders using the `id` property, as shown in Example 4.
