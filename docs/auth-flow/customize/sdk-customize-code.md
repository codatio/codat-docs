...
| `accounting.consents.title`<br/>`banking.consents.title`<br/>`commerce.consents.title` | `string` _(accepts Markdown)_ <br/><br/>Title displayed on the consent page. Accepts `{companyName}` and `{platformName}` replacements, such as `Connect your {platformName} account to {companyName}`.|
| `accounting.consents.subtitle`<br/>`banking.consents.subtitle`<br/>`commerce.consents.subtitle` | `string` _(accepts Markdown)_ <br/><br/>Subtitle displayed on the consent page. Accepts `{companyName}` and `{platformName}` replacements, such as `Allow {companyName} to access your {platformName} data`.|
