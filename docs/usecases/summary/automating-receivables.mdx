...
- `taxRates` to retrieve a list of tax rates which can be used against invoices.

Then, use the same API to create invoices and credit notes within the accounting application. Once an invoice has been created, a payment can then be written to the customer's accounting software with a payment link, allocating and reconciling a payment against an invoice, credit note, or payment on account. 
