...
description: "A guide to App Store Partner certification with Codat"
---

Codat handles many of Xero's [Certification Checkpoints](https://developer.xero.com/documentation/xero-app-store/app-partner-guides/certification-checkpoints/) for you. However, there are some checkpoints that require you to take action. The key ones that you need to focus on relate to your app's UI.
In particular:
* Checkpoint 3: Connection
* Checkpoint 4: Error Handling
* Checkpoint 8: Sign Up with Xero
and, if you are pushing data:
* Checkpoint 12 and/or 16: Account Mapping

See below for a full breakdown of what you need to do for each Certification Checkpoint. 
We recommend that you work closely with your Codat account team during your implementation of Xero's Certification Checkpoints. 
...
Your app name must reflect the go-to-market name of your app or product and cannot include the word Xero. This is important because the App Name will be visible to end users at multiple points in the customer journey. You can edit your app name in the My Apps section of your Xero Developer Portal.
...
Xero would like you to use [Xero-branded “connect” and “disconnect” buttons](https://developer.xero.com/documentation/guides/how-to-guides/branding-your-integration/) but this is not a hard requirement. If your app has multiple integrations, you can use a standard set of buttons for consistency of style. 

### 3. Connection

#Action Required: Significant

While Codat handles the initial connection to Xero via the Codat Link UI, Xero requires that you create a page in your app where the user can manage their connection on an ongoing basis. You should use this page to satisfy the following requirements:
...
| Include a button to disconnect | When a user clicks on your button, use Codat's PATCH connections endpoint to disconnect. |
| Handle a disconnect from Xero's side | Codat handles the disconnection from Xero’s end. You need to identify when a disconnect has occurred by using the alert of type “Data Connection Status Changed”. When this alert is triggered, change the connection status in your UI and display a "Reconnect" or "Connect" button. |
...
If an error occurs, your UI should communicate the fact that an error has occurred to the user. Detailed error messages or logs are a best practice but not a hard requirement. Details on the types of error messages provided by Codat is available [here](https://docs.codat.io/using-the-api/errors).
...
All apps seeking certification and listing in the Xero App Store need to build a Sign Up with Xero flow.

Our full guide to Sign Up with Xero can be found [here](/integrations/accounting/xero/xero-sign-up-with-xero).
...
If you are posting or putting data to Xero, you need to create an account mapping interface for your users to use. 
Before you create your account mapping interface, we recommend you review Xero's [best practice guide](https://developer.xero.com/documentation/guides/how-to-guides/integration-best-practices) and Codat's [implementation guide](https://docs.codat.io/using-the-api/best-practices/implementing-a-mapping-page#docusaurus_skipToContent_fallback).

### 13. Currency

#Action Required: None or Minimal

This checkpoint is only relevant if your app deals in multiple currencies. 
...
Codat handles the rounding of totals and ensures they match what is in Xero.
  
### 15. Taxes

#Action Required: None or Minimal

If your app handles tax in any way you need to ensure the correct tax rate is used on invoices, quotes etc. 

Codat supports the pulling of tax rates as individual tax rates and on transactions (invoices, payments etc).

We support the pushing of transactions where a tax rate must be defined with us handling any calculations such as compound tax rates where there are multiple tax components which must be applied to calculate the effective tax rate.

We do not currently support creating new tax rates, only existing rates in a SMBs Xero account may be used. You may need to build out the mapping of tax rates in your configuration page (please see Checkpoint 12).
...
You can use Codat’s Push options endpoint to show the options available to allow this building of a mapping interface. Codat recommends a configuration page in which the SME can choose where to map everything (please see Checkpoint 12)
