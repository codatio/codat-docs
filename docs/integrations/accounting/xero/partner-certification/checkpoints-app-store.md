...
Listing in the Xero App Store is only required for Xero's Enterprise partner tier. If you are on a lower partner tier and not planning to list in the App Store, you can ignore this requirement.

All apps seeking certification and listing in the App Store must create a user journey that takes potential customers from the Xero App Store to their app. The purpose of this is two-fold:
...
To be listed on the [Xero App Store](https://apps.xero.com/), you need to be in Xero's Plus tier or higher. The listing process requires you to [create and submit your app listing for review](https://developer.xero.com/documentation/xero-app-store/app-partner-guides/app-listing).
