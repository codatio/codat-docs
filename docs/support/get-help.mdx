---
title: Get help
description: Review the support options available to you
hide_table_of_contents: true
displayed_sidebar: docs
tags:
  - Codat
  - Support
  - Get Help
---

<div className="container">
  <ul className="card-container">
    <li className="card animation-pulse">
      <div className="header">
        <img src="/img/wp-icons/accounting-2.png" className="mini-icon" />
        <h3>Raise docs issue</h3>
      </div>
      <p>
        Raise an issue with documentation and API reference on our {" "}
        <a href="https://github.com/codatio/codat-docs/issues/new?assignees=&labels=&projects=&template=issue-with-codat-docs.md&title=">GitHub</a>.
      </p>
    </li>
    <li className="card animation-pulse">
      <div className="header">
        <img src="/img/wp-icons/accounting-2.png" className="mini-icon" />
        <h3>Get technical support</h3>
      </div>
      <p>
        <a href="https://codat.zendesk.com/hc/en-gb/requests/new">Raise a technical issue</a>{" "} with our support team.
      </p>
    </li>
    <li className="card">
      <div className="header">
        <img src="/img/wp-icons/Stack.png" className="mini-icon" />
        <h3>Request a feature</h3>
      </div>
      <p>
        <a
          href="https://docs.google.com/forms/d/e/1FAIpQLSfngaDxM3kPQXT_GtIt9MOR55KYRPydjSnHlvYgtSskIM5oDg/viewform"
          target="_blank"
        >Raise a feature request</a>{" "} with our product team.
      </p>
    </li>
    <li className="card">
      <div className="header">
        <img src="/img/wp-icons/Coins.png" className="mini-icon" />
        <h3>Sales inquiry</h3>
      </div>
      <p>
        <a href="https://www.codat.io/plans/#get-in-touch" target="_blank">Contact our sales team</a>{" "}to schedule a demo and learn about Enterprise pricing opportunities.
      </p>
    </li>
    <li className="card">
      <div className="header">
        <img src="/img/wp-icons/CreditCard.png" className="mini-icon" />
        <h3>Billing inquiry</h3>
      </div>
      <p>
        <a href="mailto:AR@codat.io">Raise a billing inquiry</a>{" "} with our accounts team.
      </p>
    </li>
    <li className="card dark hypercard">
      <div className="header">
        <img src="/img/cochat/logo-openai-mini.png" className="mini-icon" />
        <h3>Ask Cochat</h3>
      </div>
      <p>
        Get instant assistance with{" "}<a href="https://cochat.codat.io" target="_blank">Cochat,</a>{" "}
        our beta AI tool trained on the Codat docs. Try{" "}<a href="https://cochat.codat.io" target="_blank">asking it a question</a>{" or "}<a href="/cochat">read more</a>.
      </p>
    </li>
  </ul>
</div>
