...
|**4** | Configure webhooks for new event types | **Standard products**: subscribe to `read.completed` event types. <br/> **Custom products**: subscribe to product-specific `read.completed` event types. |   | 1. Unsubscribe from the new event types<br/>2.Resubscribe to previously disabled webhook event types<br/>3. Re-enable client-level sync settings |
