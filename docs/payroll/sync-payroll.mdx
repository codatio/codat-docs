...
Before you can record the payroll in the accounting software, you'll need to generate a line item object for each employee for each corresponding payroll component. That means you'll likely have multiple line items for each employee.
...
In order to record payroll entries in the accounting software, you can use the [create journal entries](/sync-for-payroll-api#/operations/create-journal-entry) endpoint to push payroll to the mapped accounts on the SMB's general ledger.
...
The response indicates that the following properties must be populated (`required: true`) when you create a journalEntry to represent payroll in Xero:
- `postedOn` 
...
After your request has been accepted, it will have a status of `Pending`. You should use the [PushOperationStatusChanged](../using-the-api/webhooks/event-types) event type to track when the status of your pushOperation changes to `Success` or `Failed`.
