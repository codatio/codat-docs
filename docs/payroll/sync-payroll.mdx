...
should use the
[{dataType}.write.successful and {dataType}.write.unsuccessful](../using-the-api/webhooks/event-types)
event types to track when the status of your pushOperation changes to `Success`
or `Failed`.
